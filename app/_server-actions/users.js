...
export async function createUser(values) {
    ...
    try {
        const cookieStore = await cookies();
        const userCookie = cookieStore.get("user");
        const users = await Axios.post("https://sand.libanoinstituto.com.br/create-user", {
            name: values.name,
            email: values.email,
...
export async function deleteUser(id) {
    ...
    try {
        const cookieStore = await cookies();
        const userCookie = cookieStore.get("user");
        const users = await Axios.delete(`https://sand.libanoinstituto.com.br/delete-user/${id}`, {
            headers: {
                "Key": userCookie?.value
            }
        });
...
export async function updateUser(id, values) {
    ...
    try {
        const cookieStore = await cookies();
        const userCookie = cookieStore.get("user");
        const users = await Axios.put(`https://sand.libanoinstituto.com.br/update-user/${id}`, {
            name: values.name,
            email: values.email,
            pass: values.password,
            permission: values.permission,
